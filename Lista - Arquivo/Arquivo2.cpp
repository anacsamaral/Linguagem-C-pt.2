...
int BuscaAlunoRA(FILE *PtrAluno, char RA[14]) //arquivo passa aberto
{
	...
	//while(!feof(PtrAluno) && !(strcmp(ChaveRA, Reg.RA) == 0 && R.Status))
	//while(!feof(PtrAluno) && (strcmp(ChaveRA, Reg.RA) != 0 && R.Status != 0))
	//enquanto nao for o final do arquivo e RA diferente de zero ou Status diferente de zero
...
{
	Aluno R;
	FILE *PtrAlu = fopen("AlunosLOG.dat", "rb"); //ponteiro esta no inicio;
...
// === EXCLUSï¿½O ===
/*	Criar um arquivo novo que receberï¿½ as informaï¿½ï¿½es do antigo arquivo
	Deletar o antigo arquivo e renomear o atual -> remove("AlunosLOG.dat")
	Renomear o arquivo atual -> rename("Temp.dat", "AlunosLOG.dat"))
