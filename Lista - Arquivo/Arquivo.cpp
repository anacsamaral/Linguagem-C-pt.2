...
char Menu(void)
{
	...
	printf("\n[D] Alterar por R.A.");
	printf("\n[E] Ordenar por R.A.");
	printf("\n[F] Excluir por R.A.(Fisica)");
	printf("\n[G] Excluir por R.A.(Logica)");
	printf("\n[ESC] Finalizar");
	printf("\nOpcao Desejada: ");
...
void OrdenarAlunos(void)
{
	...
	else
	{
		...
		for(a=0; a<QtdeReg-1; a++) //a++ --> a += sizeof(TpAluno); b < QtdeReg - sizeof
			...
			*/
			{
				fseek(PtrAlu,a*sizeof(Aluno), 0);//fseek n�o funciona sozinho
...
void GravarAluno(void)
{
	...
	//Ponteiro s� vai existir se a fun��o for chamada;
	FILE *Ptr = fopen("Alunos.dat", "ab+"); //Sempre ser� esse nome
	//ab+ para deixar ler, da para usar a busca dentro;
	//� recomendado abrir com rb ou rb+
	printf("\n## Cadastro de Alunos ##\n");
	printf("\nR.A.: "); fflush(stdin);
	gets(Reg.RA);
...
void ExibirAluno(void)
{
	...
	//VALIDANDO SE O ARQUIVO EXISTE (n�o tem comando, mas)
	if(PtrAlu == NULL) //null = endere�o nulo/vazio;
	//se sim, significa que foi corrompido ou n�o existe.
		printf("\nErro de Abertura!\n");
	else
	{
		fread(&R, sizeof(Aluno), 1, PtrAlu);
		while(!feof(PtrAlu)) //feof = end of file, n�o � fim de arquivo? vc leu o fim de arquivo?
		...
		//se nao foi aberto, nao pode ser fechado, s� da fclose se foi aberto
		fclose(PtrAlu);
	}
	getch();
	//EOF � usado com arquivo texto;
}
	...
	else
	{
		...
		while(strcmp(RegAluno.RA, "\0")!=0)
		{
			...
			else
			{
				...
				//SEEK_SET � a partir do inicio do arq; 
				//SEEK_CUR � a partir de onde o ponteiro esta;
				//SEEK_END � um valor positivo para tras, tipo, 96 bytes para tras;
				eles s�o constantes e precisam ser maiusculas
		...
		while(strcmp(RegAluno.RA, "\0")!=0)
		{
			...
			else
			{
				...
				if (toupper(getche()) == 'S')
				{
					...
					fseek(PtrAluno, pos, 0); //ponteiro vai para a posi��o que achou na busca
...
// === EXCLUSAO ===
/*	Criar um arquivo novo que receber� as informa��es do antigo arquivo
